...
AgentLens is a monorepo with two publishable packages and a strict dependency
...
agentlens/
  packages/
    core/          @agentlens/core — scanner library
      src/
    ...
    cli/             agentlens — argument parsing + terminal rendering only
  fixtures/          per-agent repos + shared fake $HOME (AGENTLENS_HOME)
...
- **Testability.** Home-directory access goes through `effectiveHomeDir()`,
  which honours `AGENTLENS_HOME`, so integration tests run against
